...
<%
	...
	if((pathMap == null) || (pathMap == ""))
		...
		pathMap = "mstrmaps2/" + pathMap; //chiamata a pagina custom
	
	if((pathMap==null)||(pathMap==""))
		pathMap="mstrmaps2/1.jsp"; //pagina iniziale di default
...
else
{
	try {
		...
		if (SvgMapsUtils.checkSvgMaps(context, true)) {
			
			// check if the files have been compressed (gzipped) by atvise builder. If so, decompress them before loading
			SvgMapsUtils.unzipMaps(SvgMapsUtils.getSvgMapsRoot(context, true));
			
		    SvgMapsUtils.modifyWebMIContext(SvgMapsUtils.getSvgMapsRoot(context, true) +"/webmi.js");
		    
		    svgMapInstalled = true;
		    
		    // verify if Atvise pages are translated in the language on session. If not, use EN by default.
		    if (SvgMapsUtils.checkLanguageTranslation(lang_code, context, true))
	...
	if (svgMapInstalled)
    {
	 // we include the SVG maps only if they're actually there. %> 
	    	<iframe id="map" src="./app/mstrmaps2/svgmaps/index.htm?language=<%=lang_request%>&autofit=true&displaytype=svg&scrolling=no" style="position:fixed;top:0px;left:0px;width:100%;border:0px;margin:0px;padding:0px;"></iframe>
			<% if (bValueNote) { %>
